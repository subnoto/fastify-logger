...
const { serverFactory, TIME, unmockTime, mockTime, serializeError, CustomError } = require('./helpers');
...
test('should log an error message', async (t) => {
  t.beforeEach(async () => {
    server = serverFactory(messages, { minimumLevel: 'trace' });

    server.get('/a-path-with-error', (req, res) => {
      req.log.error({ error: serializeError(new CustomError('my custom error')) }, 'a user defined error log');
      res.send();
    });

    await server.inject('/a-path-with-error');
  });
  ...
  await t.test(
    ...
    (t) => {
      const messagesExpected = [
        `${TIME} - info - GET /a-path-with-error - incoming request\n`,
        `${TIME} - error - a user defined error log\n{}\n`,
        `${TIME} - info - request completed\n`
      ];
      t.assert.deepStrictEqual(messages, messagesExpected);
